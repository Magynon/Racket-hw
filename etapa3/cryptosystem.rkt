...
(define FList (list a1 b1 c1 a2 b2 c2 a3 b3 c3))
...
(define (key n)
  (let ([e (list-ref (get-nth-quadruple n) 1)] [f (list-ref (get-nth-quadruple n) 2)])
    (map (λ (a)
           (let ([B (remainder a 27)])
             (if (< B 0) (+ 27 B) B)
             )
           )
         (foldl (λ (func ans) (append ans (list (func e f))))
           null
           FList
           ))
    )
...
(define (cut key size ans)
  (if (zero? size)
      ans
      (cut (cdr key)
           (sub1 size)
           (append ans (list (car key)))
         )
      )
  )

(define (elongate key size)
  (if (> (length key) size)
      key
      (elongate (append key key) size)
      )
  )

(define (extend-key key size)
  (cond [(> (length key) size)
         (cut key size null)
         ]
        [(< (length key) size)
         (cut (elongate key size)
              size
              null
           )
         ]
...
(define (auxF op message key)
  (let ([key (extend-key key (length message))])
    (map (λ (t1 t2) (let ([abc (remainder (op t1 t2) 27)])
                      (if (< abc 0) (+ abc 27) abc)
                      ))
         message
         key
