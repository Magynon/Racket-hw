...
(define (compute-children tuple)
  (list (multiply T1 tuple) (multiply T2 tuple) (multiply T3 tuple))
  )

(define (queue-aux queue)
  (let [(current (stream-first queue))]
    (stream-cons
     current
     (queue-aux (stream-append (stream-rest queue) (compute-children current)))
     )
    )
  )
...
(define G
  (let nat ([seed 1])
    (stream-cons seed (nat (add1 (add1 seed))))))
...
(define gh-pairs-stream
  (stream-filter (λ (tuple)
                   (if (equal? 1 (gcd (cdr tuple) (car tuple))) #t #f)
                   )
                 (pairs G H))
)
